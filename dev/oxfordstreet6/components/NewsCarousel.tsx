...
export function NewsCarousel() {
  ...
  return (
    <section aria-labelledby="latest-news" className="section">
      <div className="container">
        <div style={{display:'flex', alignItems:'baseline', justifyContent:'space-between'}}>
          <h2 id="latest-news">News & Updates</h2>
          <div aria-live="polite" className="kicker">Item {index+1} of {items.length}</div>
        </div>
          ...
          >
            {items.map((n, i) => (
              <Link
                ...
                data-idx={i}
                className="card"
                style={{padding:16, borderWidth: i===index ? 2 : 1, borderColor: i===index ? 'var(--gold)' : 'rgba(17,19,22,0.06)'}}
                onKeyDown={(e) => {
