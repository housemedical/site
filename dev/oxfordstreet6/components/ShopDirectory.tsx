...
export function ShopDirectory() {
  ...
  const [active, setActive] = useState<Shop | null>(null);
  ...
  const filtered = useMemo(() => {
    return shops.filter(s =>
      (!cat || s.category === cat) &&
      (q.trim().length === 0 || s.name.toLowerCase().includes(q.toLowerCase()))
    );
  }, [q, cat]);

  function openModal(s: Shop) {
    setActive(s);
    const d = dialogRef.current;
    if (!d) return;
    d.showModal();
    document.body.style.overflow = 'hidden';
    setTimeout(() => d.querySelector<HTMLElement>('h3')?.focus(), 0);
  }
  function closeModal() {
    const d = dialogRef.current;
    if (!d) return;
  ...
  return (
    <section aria-labelledby="shops" className="section">
      <div className="container">
        ...
        <div className="card" style={{padding:16}}>
          <form role="search" aria-label="Search shops" className="hairline" style={{display:'grid', gridTemplateColumns:'1fr auto', gap:12, paddingTop:12}} onSubmit={e=>e.preventDefault()}>
            ...
            />
            <select aria-label="Filter by category" value={cat ?? ''} onChange={e=> setCat(e.target.value || null)} style={{padding:12, borderRadius:12, border:'1px solid rgba(17,19,22,0.12)'}}>
          ...
          <div>
            <div style={{display:'grid', gridTemplateColumns:'1.2fr 1fr', gap:0}}>
              ...
              <div style={{padding:16}}>
                ...
                <h3 tabIndex={-1} style={{marginTop:8}}>{active.name}</h3>
                <p>{active.description}</p>
                <p><strong>Address:</strong> {active.address}</p>
